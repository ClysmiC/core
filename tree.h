...
//  library, since most tree building use-cases won't need it. We could probably just make each UiControl
...
struct TreeBuilder
{
    T * root;
    T * currentParent;
    T * insertChildAfter; // aka "cursor". if null inserts at currentParent.firstChild, else inserts after the cursor
...
BeginRoot(TreeBuilder<T> * builder, T * root)
{
    ...
    Assert(builder->currentParent == nullptr);
    Assert(builder->insertChildAfter == nullptr);
    
    builder->root = root;
    builder->currentParent = root;
    builder->insertChildAfter = nullptr;
}
...
{
    Assert(!builder->isFinished);
    Assert(builder->currentParent == builder->root);
...
BeginChild(TreeBuilder<T> * builder, T * child)
{
    ...
    if (isRebuilding)
    {
        Assert(*PMember(child, builder->mParent) == builder->currentParent);
...
{
    Assert(builder->currentParent != builder->root); // Should you be calling EndRoot instead?

    builder->insertChildAfter = builder->currentParent;
    builder->currentParent = *PMember(builder->currentParent, builder->mParent);
}
...
template <typename T>
internal void
SetInsertionCursorAfter(TreeBuilder<T> * builder, T * insertChildAfter)
{
    // NOTE - passing nullptr for insertChildAfter makes the next inserted
    //  item become the head of the list
    
    builder->insertChildAfter = insertChildAfter;
}
